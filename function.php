...
if (isset($_POST['addproduk'])) {
    ...
    $stocksekarang = $hitung2['stock']; //hitung barang saat ini
...
if (isset($_POST['editpelanggan'])) {
    ...
    if ($query) {
        ...
        echo
        '<script>alert("Gagal");
      window.location.href="pelanngan.php"
      </script> 
      ';
    }
}
...
//menambah profit
if(isset($_post['tambahprofit'])){
    $idproduk = $_POST['idproduk'];
    $modalbeli = $_POST['modalbeli'];
    $hargajual = $_POST['hargajual'];

    $insert = mysqli_query($c, "insert into profit (idproduk,modalbeli,hargajual) values('$idproduk','$modalbeli','$hargajual')");

    if ($insert) {
        header('location:profit.php');
    } else {
        echo
        '<script>alert("gagal menambah profit baru");
